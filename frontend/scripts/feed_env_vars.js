...
const allEnvs = ['development', 'test', 'staging', 'production'];
...
Object.keys(process.env).forEach((key, index, keys) => {
  let parsedKey = key;
  if (!skipKey(key)) {
    if (parsedKey.startsWith(`${ENV}_`)) {
      parsedKey = parsedKey.substring(ENV.length + 1);
    } else if (keys.includes(`${ENV}_${key}`)) {
      // Prevents a default key from overwriting its environment-specific key, if it exists.
      return;
    }
